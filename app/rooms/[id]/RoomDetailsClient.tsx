...
export default function RoomDetailsClient() {
    ...
    useEffect(() => {
        ...
        const fetchRoom = async () => {
            try {
                setLoading(true);
                const backendRoom = await apiService.getRoom(id as string);
                
                setRoom(backendRoom);
                setActiveImage(backendRoom.mainImage);
                setError(null);
            } catch (err) {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-background">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest text-[#292f36]">Loading Experience...</p>
                </div>
            </div>
        );
    }
                        ...
                        <div className="lg:col-span-7 space-y-6">
                            ...
                            <div className="grid grid-cols-4 gap-3">
                                {allImages.map((img, idx) => (
                                    ...
                                    >
                                        <Image src={resolveImageUrl(img)} alt={`${room.name} gallery ${idx}`} fill className="object-cover" />
                                    </button>
                                ))}
                            </div>
                        </div>
